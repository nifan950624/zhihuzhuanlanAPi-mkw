...
const createRequest = (method, url, data, options = {}) => {
  return new Promise((resolve, reject) => {
    let headers = { 'User-Agent': chooseUserAgent(options.ua), ...options.headers }
    if (method.toUpperCase() === 'POST') {
      headers['Content-Type'] = 'application/x-www-form-urlencoded'
    }
    if (url.includes('zhihu.vikingship.xyz')) {
      headers['Referer'] = 'http://zhihu.vikingship.xyz'
    }

    let settings = {
      method: method,
      url: 'http://api.vikingship.xyz/api' + url,
      headers: headers,
